...
# 전종목 당일 시세 (OHLCV + 거래대금 + 등락률)
snapshot = get_kr_market_snapshot(date="20240423", market="KOSPI")

# 개별 종목 펀더멘탈 (PER, PBR, DIV) — 종목 단위로 조회
# 주의: get_kr_fundamental은 ticker 필수. 전종목 일괄 조회는 미지원.
# 스크리닝 시에는 snapshot 결과에서 후보를 먼저 좁힌 뒤,
# 상위 종목에 대해서만 get_kr_fundamental을 개별 호출한다.
fund = get_kr_fundamental(ticker="005930", from_date="20240101", to_date="20240423")
```
